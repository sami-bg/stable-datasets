...
def build_probe(embed_dim: int, num_classes: int, protocol: str = "common") -> nn.Module:
    """Barlow Twins's probe head.

    Native: a bare linear layer on the backbone's final global-average-pooled
    feature, with NO extra probe-side BatchNorm. Barlow Twins does use BatchNorm inside
    ...
    raise ValueError(f"barlow_twins: unknown probe protocol {{protocol!r}}")
