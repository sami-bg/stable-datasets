...
class MosquitoSound(BaseDatasetBuilder):
    ...
    SOURCE = DatasetSource(
        ...
        assets={
            "train": DownloadInfo(url="http://www.timeseriesclassification.com/Downloads/MosquitoSound.zip"),
            "test": DownloadInfo(url="http://www.timeseriesclassification.com/Downloads/MosquitoSound.zip"),
        },
    ...
    def _info(self):
        return DatasetInfo(
            description="MosquitoSound audio-derived timeseries classification dataset.",
            features=Features(
                {
    ...
    def _generate_examples(self, data_path, split):
        with zipfile.ZipFile(data_path) as archive:
            member = _find_member(archive, f"MosquitoSound_{split.upper()}.arff")
