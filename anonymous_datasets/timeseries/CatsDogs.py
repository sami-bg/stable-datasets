...
class CatsDogs(BaseDatasetBuilder):
    """CatsDogs timeseries classification dataset."""

    VERSION = Version("1.0.0")
    SOURCE = DatasetSource(
        homepage="http://www.timeseriesclassification.com/description.php?Dataset=CatsDogs",
        assets={
            "train": DownloadInfo(url="http://www.timeseriesclassification.com/Downloads/CatsDogs.zip"),
            "test": DownloadInfo(url="http://www.timeseriesclassification.com/Downloads/CatsDogs.zip"),
        },
        citation="N/A.",
    )
    ...
    def _info(self):
        return DatasetInfo(
            description="CatsDogs audio-derived timeseries classification dataset.",
            features=Features(
                {
    ...
    def _generate_examples(self, data_path, split):
        with zipfile.ZipFile(data_path) as archive:
            member = _find_member(archive, f"CatsDogs_{split.upper()}.arff")
