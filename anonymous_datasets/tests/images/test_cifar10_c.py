...
from anonymous_datasets.images import CIFAR10C


def test_cifar10_c_dataset():
    # CIFAR10C(split="test") automatically downloads and loads the dataset
    cifar10_c = CIFAR10C(split="test")

    # Test 1: Check that the dataset has the expected number of samples
    # CIFAR-10-C has 19 corruptions * 5 levels * 10,000 images = 950,000 samples
    expected_num_test_samples = 19 * 5 * 10000
    assert len(cifar10_c) == expected_num_test_samples, (
        f"Expected {expected_num_test_samples} test samples, got {len(cifar10_c)}."
    )
    ...
    expected_corruptions = CIFAR10C._corruptions()
    ...
    print("All CIFAR-10-C dataset tests passed successfully!")
