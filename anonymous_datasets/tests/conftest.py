...
is added to sys.path, allowing tests to import from the anonymous_datasets package
...
def _anonymous_datasets_cache_dirs():
    """Default cache dirs used by dataset builders (downloads + processed)."""
    base = Path(os.path.expanduser("~/.anonymous_datasets"))
    return [base / "downloads", base / "processed"]


@pytest.fixture(autouse=True, scope="function")
def cleanup_anonymous_datasets_cache_after_test():
    """Remove anonymous_datasets cache dirs after each test when running in CI.
    ...
    if os.environ.get("CI") != "true" and os.environ.get("GITHUB_ACTIONS") != "true":
        return
    for d in _anonymous_datasets_cache_dirs():
        if d.exists():
