...
def test_controlled_transforms(our_transform, true_transform):
    ...
    our_dataset = apl.data.dataset.DictFormat(CIFAR10("~/data", download=True))
    our_dataset = apl.data.dataset.AddTransform(our_dataset, transform)
    t = v2.Compose(
...
def test_transforms_performance():
    ...
    dataset = apl.data.dataset.DictFormat(CIFAR10("~/data", download=True))
    dataset = apl.data.dataset.AddTransform(dataset, transform)
