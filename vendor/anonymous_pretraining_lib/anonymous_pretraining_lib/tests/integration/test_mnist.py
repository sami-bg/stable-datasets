...
class TestMNISTIntegration:
    ...
    def test_dictconfig_with_manager(self):
        ...
        data = OmegaConf.create(
            {
                "_target_": "anonymous_pretraining_lib.data.DataModule",
                "train": {
                    "dataset": {
    ...
    def test_datamodule_with_mnist(self):
        ...
        train = dict(
            dataset=apl.data.HFDataset(
                path="ylecun/mnist",
                split="train",
                transform=apl.data.transforms.ToImage(),
            ),
            batch_size=512,
            shuffle=True,
            num_workers=10,
        ...
        # Create DataModule
        data = apl.data.DataModule(train=train, test=test)
    ...
    def test_mnist_training_with_resnet9(self):
        ...
        # Create data module
        data = apl.data.DataModule(train=train, val=val)
        ...
        backbone = apl.backbone.Resnet9(num_classes=10, num_channels=1)
        module = apl.Module(backbone=backbone, forward=forward)
    ...
    def test_mnist_dataset_properties(self):
        ...
        train_dataset = apl.data.HFDataset(
            path="ylecun/mnist", split="train", transform=apl.data.transforms.ToImage()
        )

        # Load MNIST test dataset
        test_dataset = apl.data.HFDataset(
            path="ylecun/mnist", split="test", transform=apl.data.transforms.ToImage()
        )
