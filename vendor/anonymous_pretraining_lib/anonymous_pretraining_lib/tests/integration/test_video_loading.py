...
import anonymous_pretraining_lib
...
class TestVideoLoadingIntegration:
    ...
    def test_clip_extract(self):
        ...
        dataset = anonymous_pretraining_lib.data.HFDataset(
            path="shivalikasingh/video-demo",
            split="train",
            trust_remote_code=True,
            transform=anonymous_pretraining_lib.data.transforms.RandomContiguousTemporalSampler(
    ...
    def test_clip_dataset_with_dataloader(self):
        ...
        dataset = anonymous_pretraining_lib.data.HFDataset(
            ...
            split="train",
            trust_remote_code=True,
            transform=anonymous_pretraining_lib.data.transforms.Compose(
                anonymous_pretraining_lib.data.transforms.RandomContiguousTemporalSampler(
                    source="video", target="video", num_frames=10
                ),
                anonymous_pretraining_lib.data.transforms.Resize(
    ...
    def test_embedding_from_video_frames(self):
        ...
        dataset = anonymous_pretraining_lib.data.HFDataset(
            ...
            transform=anonymous_pretraining_lib.data.transforms.Compose(
                ...
                anonymous_pretraining_lib.data.transforms.ToImage(
                    scale=False,
                    mean=[0, 0, 0],
                    std=[255, 255, 255],
                    source="video",
        ...
        embedding = anonymous_pretraining_lib.utils.ImageToVideoEncoder(
    ...
    def test_video_encoder_with_different_backbones(self):
        ...
        for backbone in backbones:
            encoder = anonymous_pretraining_lib.utils.ImageToVideoEncoder(backbone)
