...
class TestSimCLRIntegration:
    ...
    def test_multi_view_data_loading(self):
        ...
        sampler = apl.data.sampler.RepeatedRandomSampler(dataset, n_views=2)
    ...
    def test_fold_views_operation(self):
        ...
        # Fold views
        views = apl.data.fold_views(features, sample_idx)
    ...
    def test_temperature_scaling(self):
        ...
        for temp in [0.05, 0.1, 0.5, 1.0]:
            loss_fn = apl.losses.NTXEntLoss(temperature=temp)
            loss = loss_fn(z1, z2)
            assert loss.item() > 0
