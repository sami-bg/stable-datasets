...
class TestDatasetIntegration:
    ...
    def test_hf_datasets(self):
        ...
        dataset1 = apl.data.HFDataset("ylecun/mnist", split="train")
    ...
    def test_hf_dataloaders(self):
        ...
        dataset = apl.data.HFDataset("ylecun/mnist", split="train", transform=t)
    ...
    def test_datamodule(self):
        ...
        # Create DataModule
        module = apl.data.DataModule(train=train, test=test, val=test, predict=test)
    ...
    def test_dataset_sizes(self):
        ...
        train_dataset = apl.data.HFDataset("ylecun/mnist", split="train")
        test_dataset = apl.data.HFDataset("ylecun/mnist", split="test")
    ...
    def test_fromtensor_dataset(self):
        ...
        trans = apl.data.transforms.ToImage(mean=(0.5,), std=(0.5,))

        # fake torch dataset
        dataset = torch.utils.data.TensorDataset(mock_data)
        data = apl.data.utils.FromTorchDataset(dataset, names=["image"])
        data_trans = apl.data.utils.FromTorchDataset(
    ...
    def test_minaristep_dataset_bounds(self):
        ...
        dataset = apl.data.MinariStepsDataset(minari_dataset, num_steps=2)
