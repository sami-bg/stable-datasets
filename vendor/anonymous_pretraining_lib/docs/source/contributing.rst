...
Thank you for your interest in contributing to ``anonymous-pretraining-lib``!
This library is a community-driven project, and we greatly appreciate contributions of all kinds.

If you encounter any issues or have suggestions, please open an issue on our `issue tracker XXXX`_. This allows us to address problems and gather feedback from the community.
...
The preferred workflow for contributing to ``anonymous-pretraining-lib`` is to fork the
`main repository XXXX`_ on
GitHub, clone, and develop on a branch. Steps:

1. Fork the `project repository <https://github.com/anonymous-org/anonymous-pretraining-lib>`_
   by clicking on the 'Fork' button near the top right of the page. This creates
   a copy of the code under your GitHub user account. For more details on
   how to fork a repository see `this guide XXXX`_.

2. Clone your fork of the ``anonymous-pretraining-lib`` repo from your GitHub account to your local disk::

      $ git clone git@github.com:YourLogin/anonymous-pretraining-lib.git
      $ cd anonymous-pretraining-lib
...
6. Follow `these instructions XXXX`_
...
- The automatic tests pass on your local machine. This can be done by running ``python -m pytest anonymous_pretraining_lib/tests -m unit`` in the root directory of the repository after making the desired changes. Only unit tests should be run locally. See `TESTING.md XXXX`_ for detailed information about writing and running tests.
...
All contributions should include appropriate tests. See `TESTING.md XXXX`_ for comprehensive guidance on writing unit tests, integration tests, and best practices for the anonymous-pretraining-lib testing framework.


New contributor tips
--------------------

A great way to start contributing to ``anonymous-pretraining-lib`` is to pick an item
from the list of `good first issues XXXX`_ in the issue tracker. Resolving these issues allows you to start
