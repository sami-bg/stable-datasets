...
## Overview
Tests in anonymous-pretraining-lib are categorized to separate fast unit tests from slow integration tests. This enables efficient CI/CD while maintaining comprehensive test coverage.
...
```
anonymous_pretraining_lib/tests/
...
### Use Test Utilities
```python
from anonymous_pretraining_lib.tests.utils import MockImageDataset, create_mock_dataloader
...
def test_loss_computation():
    ...
    loss_fn = apl.losses.NTXEntLoss(temperature=0.1)
    loss = loss_fn(z1, z2)
    assert loss.item() >= 0
```
...
```yaml
- name: Run Unit Tests
  run: python -m pytest anonymous_pretraining_lib/ -m unit --verbose --cov=anonymous_pretraining_lib --cov-report term
```
